...
class MediaController extends Controller
{
    ...
    public function img_categories_addImg($group_id) {
        $DataBag = array();
        $DataBag['parentMenu'] = 'media';
        $DataBag['subMenu'] = 'image';
        $DataBag['childMenu'] = 'imgCats';
        $DataBag['imgcat'] = ImageCategories::findOrFail($group_id);
        return view('dashboard.media.image.category_add_image', $DataBag);
    }

    public function img_categories_upImg(Request $request, $group_id) {
        ...
        if( $request->hasFile('image') ) {
            ...
            foreach( $request->file('image') as $img ) {
                ...
                if( isset($res) && $res == 1 ) {
                    $image_id = $Images->id;
                    $arrC = array();
                    $arrC['image_id'] = $image_id;
                    $arrC['image_category_id'] = $group_id;
                    array_push($catArr, $arrC);
                }
            }

            if( !empty($catArr) ) {
                ImageCategoryMap::insert($catArr);
                if( isset($res) && $res == 1 ) {
                    return back()->with('msg', 'Images Added Successfully.')
                    ->with('msg_class', 'alert alert-success');
                }
            }
        }
    ...
    public function img_galleries() {
    	$DataBag = array();
    	$DataBag['parentMenu'] = 'media';
    	$DataBag['subMenu'] = 'image';
    	$DataBag['childMenu'] = 'imgGals';
    	$DataBag['galleries'] = ImageGalleries::where('status', '!=', '3')->orderBy('created_at', 'desc')->get();
    	return view('dashboard.media.image.image_galleries', $DataBag);
    }

    public function imgGal_create() {
    	$DataBag = array();
    	$DataBag['parentMenu'] = 'media';
    	$DataBag['subMenu'] = 'image';
    	$DataBag['childMenu'] = 'imgGals';
    	return view('dashboard.media.image.image_gallery_create', $DataBag);
    }

    public function imgGal_save(Request $request) {

    	$ImageGalleries = new ImageGalleries;
    	$ImageGalleries->name = trim(ucfirst($request->input('name')));
    	$ImageGalleries->description = trim(htmlentities( $request->input('description'), ENT_QUOTES));
    	$ImageGalleries->meta_title = trim($request->input('meta_title'));
    	$ImageGalleries->meta_keywords = trim($request->input('meta_keywords'));
    	$ImageGalleries->meta_description = trim($request->input('meta_description'));
    	$ImageGalleries->created_by = Auth::user()->id;
    	$ImageGalleries->short_code = "[#Gallery_".md5( microtime(TRUE).rand(123456,999999) )."#]";
    	$res = $ImageGalleries->save();
    	if( $res ) {
    		$id = $ImageGalleries->id;
    		return redirect()->route('media_img_gals_addImg', array('id' => $id))->with('msg', 'Gallery Created Successfully')
    		->with('msg_class', 'alert alert-success');
    	}
    ...
    public function imgGal_addImg(Request $request, $gallery_id) {
    	$DataBag = array();
    	$DataBag['parentMenu'] = 'media';
    	$DataBag['subMenu'] = 'image';
    	$DataBag['childMenu'] = 'imgGals';

        $allImages = Images::where('status', '=', '1')->orderBy('created_at', 'desc')->paginate(12);
        $DataBag['allImages'] = $allImages;
    	if( $request->ajax() ) {
            return view('dashboard.media.image.render_image', compact('allImages'));    
        }

        $DataBag['galDetails'] = ImageGalleries::findOrFail($gallery_id);
    	
        $DataBag['allGroups'] = ImageCategories::where('status', '=', '1')->orderBy('created_at', 'desc')->get();
    	$imgs = DB::table('image_gallery_map as igm')->where('igm.image_gallery_id', '=', $gallery_id)
    	->join('image', 'igm.image_id', '=', 'image.id')->where('image.status', '=', '1')->orderBy('igm.id', 'desc')
    	->select('image.image', 'image.size', 'igm.id')->paginate(12);
    	$DataBag['galImages'] = $imgs;
        
    	return view('dashboard.media.image.image_gallery_add_image', $DataBag);
    }

    public function imgGal_SaveImg(Request $request, $gallery_id) {
    	$msg = "";
    	$ImageGalleries = ImageGalleries::find($gallery_id);
    	if( isset($ImageGalleries) && !empty($ImageGalleries) ) {
    		$ImageGalleries->gallery_source = trim($request->input('gallery_source'));
    		$ImageGalleries->image_category_id = trim($request->input('image_category_id'));
    		$res = $ImageGalleries->save();
    		if( $res ) {
    			if( trim($request->input('gallery_source')) == '1' ) {
    				ImageGalleryMap::where('image_gallery_id', '=', $gallery_id)->delete();
    				$msg = "Image Group Added To Image Gallery Successfully.";	
    			}
    ...
    public function ajaxGalImgUpload(Request $request) {
    	$rtn = "";
    	$gallery_id = trim($request->input('gallery_id'));
    	$insertArr = array();
    	if( $request->hasFile('image') ) {
    		foreach( $request->file('image') as $img ) {
    			$arr = array();
	        	...
	        	$res = $Images->save();
	        	if( $res ) {
	        		$arr['image_gallery_id'] = $gallery_id;
    	...
    	if( !empty($insertArr) ) {
    		ImageGalleryMap::insert($insertArr);
    		ImageGalleries::where('id', '=', $gallery_id)->update(['gallery_source' => '2', 'image_category_id' => '0']);
    		$rtn = "OK";
    	}
    ...
    public function imgGal_edit($gallery_id) {
    	$DataBag = array();
    	$DataBag['parentMenu'] = 'media';
    	$DataBag['subMenu'] = 'image';
    	$DataBag['childMenu'] = 'imgGals';
    	$DataBag['imgGal'] = ImageGalleries::findOrFail($gallery_id);
    	return view('dashboard.media.image.image_gallery_create', $DataBag);
    }

    public function imgGal_update(Request $request, $gallery_id) {
    	...
    	if( isset($ImageGalleries) && !empty($ImageGalleries) ) {
	    	...
	    	$ImageGalleries->meta_keywords = trim($request->input('meta_keywords'));
	    	$ImageGalleries->meta_description = trim($request->input('meta_description'));
	    	$ImageGalleries->status = trim($request->input('status'));
	    	$ImageGalleries->updated_by = Auth::user()->id;
	    	$ImageGalleries->updated_at = date('Y-m-d H:i:s');
	    	$res = $ImageGalleries->save();
	    	if( $res ) {
	    		return back()->with('msg', 'Image Gallery Updated Successfully.')
	    		->with('msg_class', 'alert alert-success');
	    	}
    	}
    ...
    public function imgGal_delete($gallery_id) {

    	$ImageGalleries = ImageGalleries::findOrFail($gallery_id);
    	$ImageGalleries->status = '3';
    	$res = $ImageGalleries->save();
    	if( $res ) {
    		ImageGalleryMap::where('image_gallery_id', '=', $gallery_id)->delete();
    		return back()->with('msg', 'Image Gallery Deleted Successfully.')
	    	->with('msg_class', 'alert alert-success');
    	}
    ...
    public function imgGal_delImg($gallery_id, $gallery_map_id) {
        
        $res = ImageGalleryMap::where('id', '=', $gallery_map_id)->where('image_gallery_id', '=', $gallery_id)->delete();
        if( $res ) {
...
/*********************************************** FILES ***************************************************/
    ...
    public function all_flCats() {
        ...
        $DataBag['allFCats'] = FileCategories::where('status', '!=', '3')->orderBy('created_at', 'asc')->get();
