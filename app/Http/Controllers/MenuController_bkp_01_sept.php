...
class MenuController extends Controller
{
    ...
    public function getPages(Request $request) {
    	...
    	if( $getID == 0 ) {
    		...
    		if( $getData == 'productCat' ) {
                ...
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
    			$DataBag['pages'] = $data;
    		...
    		if( $getData == 'articleCat' ) {

    			$data = \App\Models\Article\ArticleCategories::where('status', '=', '1')->where('parent_language_id', '=', '0')
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
    			$DataBag['pages'] = $data;
                $DataBag['table_type'] = 'ARTICLE_CATEGORY';
    		}
    		...
    		if( $getData == 'career' ) {
    			...
    			$data = \App\Models\Career\Career::where('status', '=', '1')->where('parent_language_id', '=', '0')
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
    			$DataBag['pages'] = $data;
                $DataBag['table_type'] = 'CAREER';
    		}
    		...
    		if( $getData == 'profileCat' ) {

    			$data = \App\Models\PeoplesProfile\PeopleProfileCategories::where('status', '=', '1')->where('parent_language_id', '=', '0')
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
    			$DataBag['pages'] = $data;
                $DataBag['table_type'] = 'PEOPLE_PROFILE_CATEGORY';
    		}

    		if( $getData == 'techres' ) {

    			$data = \App\Models\TechResource\TechResource::where('status', '=', '1')->where('parent_language_id', '=', '0')
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
    			$DataBag['pages'] = $data;
                $DataBag['table_type'] = 'TECH_RESOURCE';
    		}
    		...
    		if( $getData == 'eventCat' ) {
    			...
    			$data = \App\Models\EventCategories::where('status', '=', '1')->where('parent_language_id', '=', '0')
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
    			$DataBag['pages'] = $data;
                $DataBag['table_type'] = 'EVENT_CATEGORY';
    		}

            if( $getData == 'vidCat' ) {

                $data = \App\Models\Media\VideoCategories::where('status', '=', '1')->where('parent_language_id', '=', '0')
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
                $DataBag['pages'] = $data;
                $DataBag['table_type'] = 'VIDEO_CATEGORY';
            }

            if( $getData == 'video' ) {

                $data = \App\Models\Media\Videos::where('status', '=', '1')->where('parent_language_id', '=', '0')
                ->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
                $DataBag['pages'] = $data;
                $DataBag['table_type'] = 'VIDEOS';
            }
    	}
    	...
    	if( $getID > 0 ) {
    		...
    		if( $getData == 'cms' ) {
    			...
    			->where('parent_language_id', '=', '0')->orderBy('created_at', 'desc')->select('id', 'name', 'slug')->get();
    			$DataBag['pages'] = $data;
    ...
    public function searchPage(Request $request) {
        ...
        $data = CmsLinks::where('slug_url', 'LIKE', '%'.$src_page.'%')->take(25)->get();
            ...
            foreach( $lngnav_info as $k => $v ) {
                ...
                if( isset($NaviMaster2) && !empty($NaviMaster2) ) {
                    ...
                    if( $NaviMaster2->cms_link_id == '0' && $NaviMaster->table_id == '0' && $NaviMaster->table_type == 'MENU_CUSTOM_LINK' ) {
                        $NaviMaster2->custom_link = $page_url;
                    }
