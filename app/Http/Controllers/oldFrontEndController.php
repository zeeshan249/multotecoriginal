...
class FrontEndController extends Controller
{
    ...
    public function home( Request $request, $lng = '' ) {
        ...
        if(isset($_SERVER['HTTP_REFERER'])  ){
            ...
            if(isset($r[2])){
                ...
                if( $r[2]!='multotec.com'){
                    ...
                    $request->session()->put('referral', $_SERVER['HTTP_REFERER']); 
        ...
        $DataBag['mps'] = \App\Models\MineralProcess::where('status', '=', '1')->where('language_id', '=', $getlngid)->orderBy('display_order', 'asc')->get();
        $DataBag['minerals'] = \App\Models\Mineral::where('status', '=', '1')->where('language_id', '=', $getlngid)->orderBy('display_order', 'asc')->get();
        ...
        $DataBag['map'] = \App\Models\HomeMap::where('language_id', '=', $getlngid)->first();
    ...
    public function webinar( $lng ) {
        ...
        if( isset($_GET['webinar_category']) && $_GET['webinar_category'] != '' ) {
            $query = $query->where( function($query) {
                $query = $query->where( 'webinar_category', '=', trim($_GET['webinar_category']) );
            } );
        }
