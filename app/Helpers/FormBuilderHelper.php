					...
					if( !empty( $sarr ) ) {
						...
						foreach( $sarr as $op ) {
							...
							if( $data->font != '' ) {
								...
								$stylechk .= "font-family:".$data->font."!important;";	
							}
							if( $data->size != '' && $data->size != 0 ) {

								$stylechk .= "font-size:".$data->size."px!important; ";	
							}
							 
			 
  
							$fhtml .= "<p style='".$stylechk."'  ><input type='checkbox' ";
							if( $data->field_name != '') {
	...
	if( $form_id != '' ) {
		...
		if( !empty($formData) && !empty($formSettings) ) {
			...
			$frhtml .= ">";
				...
				$frhtml .= "<input type='hidden' name='referral' id='referral'>";
