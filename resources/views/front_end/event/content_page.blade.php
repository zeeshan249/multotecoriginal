...
@php
    $banner = getEventPageBanner( $allData->id );
@endphp    
...
<div class="breadcrumb">
    <ul>
        ...
        <li><a href="{{ route('eventLists', array('lng' => 'en')) }}">Events</a></li>
...
<div class="row">
    <div class="col-sm-8">
        <div class="midblock" id="firstBlock">
            ...
            @php
                $evtImage = getEventImage($allData->id);
            @endphp
            <!-- Title -->
            <h1 @if(isset($evtImage) && !empty($evtImage)) class="article_heading" @endif>
                @if( isset($allData->name) ){{ $allData->name }}@endif
            </h1>
            
            @if(isset($evtImage) && !empty($evtImage))
            <img src="{{ asset('public/uploads/files/media_images/'. $evtImage['image']) }}" alt="{{$evtImage['alt_tag']}}" 
            title="{{$evtImage['title']}}" caption="{{$evtImage['caption']}}" style="width: 100%;">
            @endif
                        ...
                        <div class="bhoechie-tab-content @if( $i == '0' ) active @endif">
                            <ul>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '01') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=01">January</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '02') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=02">February</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '03') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=03">March</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '04') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=04">April</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '05') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=05">May</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '06') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=06">June</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '07') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=07">July</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '08') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=08">August</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '09') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=09">September</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '10') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=10">October</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '11') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=11">November</a>
                                </li>
                                <li @if( isset($_GET['month']) && $_GET['month'] == '12') class="active" @endif>
                                    <a href="{{ route('eventLists', array('lng' => $lng)) }}?year={{ $yl }}&month=12">December</a>
                                </li>
                            </ul>
                        </div>
                        @php $i++; @endphp
