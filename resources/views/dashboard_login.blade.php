...
<div class="login-box" style="position: absolute; top: 0px; left: 35%; z-index: 9999;">
  ...
  <!-- /.login-logo -->
  <div class="login-box-body" style="box-shadow: 0px 0px 2px black;">
    @if(Session::has('msg'))
    <div class="alert alert-danger" style="text-align: center; padding: 4px;">{{ Session::get('msg') }}</div>
    @endif
    <p class="login-box-msg">
    <span style="font-size: 18px;"><strong>{{ $siteName }}</strong></span><br/>Sign in</p>

    <form name="frm" id="frmx" action="{{ route('dashboard_login_action') }}" method="post" autocomplete="off">
    {{ csrf_field() }}
      <div class="form-group has-feedback">
        <input type="email" name="email_id" class="form-control" placeholder="Email" autocomplete="off" value="@if(isset($_COOKIE) && !empty($_COOKIE['multotec_admin_email'])){{ $_COOKIE['multotec_admin_email'] }}@endif">
        <span class="glyphicon glyphicon-envelope form-control-feedback"></span>
      </div>
      <div class="form-group has-feedback">
        <input type="password" name="password" class="form-control" placeholder="Password" autocomplete="off" value="@if(isset($_COOKIE) && !empty($_COOKIE['multotec_admin_password'])){{ $_COOKIE['multotec_admin_password'] }}@endif">
      ...
      <div class="row">
        <div class="col-xs-8">
          <div class="checkbox icheck">
            <label>
              <input type="checkbox" name="rm_me" value="1" @if(isset($_COOKIE) && !empty($_COOKIE['multotec_admin_email']) && !empty($_COOKIE['multotec_admin_password'])) checked="checked" @endif> Remember Me
            </label>
          </div>
        </div>
        <!-- /.col -->
    ...
    <a href="{{ route('reset_link') }}">I forgot my password</a><br>
...
<!-- scripts -->
<script src="{{ asset('public/assets/particle/js/particles.js') }}"></script>
<script src="{{ asset('public/assets/particle/js/app.js') }}"></script>
<script>
$(function () {
  $('input').iCheck({
