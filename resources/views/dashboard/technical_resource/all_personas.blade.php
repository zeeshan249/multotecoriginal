...
<section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li><a href="{{ route('addPersona') }}">Add New Category</a></li>
      </ol>
    </section>
@endsection
...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('addPersona') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add New Category</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  ...
  <div class="box" style="margin-top: 10px;">
    <form name="frmx" action="{{ route('tecrp.blkAct') }}" method="post">
    {{ csrf_field() }}
    ...
    <div class="box-body">
      <table class="table table-bordered table-hover table-striped display nowrap ar-datatable" style="width:100%">
        ...
        @if(isset($allPersona))
          @php $sl = 1; @endphp
          @forelse($allPersona as $ps)
            @php
              $lngcode = getLngCode($ps->language_id);
            @endphp
          <tr>
            <td>
              {{ $sl }}
              <input type="checkbox" name="ids[]" class="ckbs" value="{{ $ps->id }}">
            </td>
            <td>
              <a href="{{ route('editPersona', array('id' => $ps->id)) }}"><i class="fa fa-pencil-square-o fa-2x base-green"></i></a>
              <a href="{{ route('delPersona', array('id' => $ps->id)) }}" onclick="return confirm('Sure To Delete This Persona ?');"><i class="fa fa-trash-o fa-2x base-red"></i></a>
              <a href="{{ route('viewTechResLst', array('lng' => $lngcode)) }}?catid={{ $ps->id }}" target="_blank">
              <i class="fa fa-eye fa-2x" aria-hidden="true"></i></a>
            </td>
            <td>
              @if($ps->status == '1')
                <a href="{{ route('acInac') }}?id={{ $ps->id }}&val=2&tab=personas"> 
                  <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i> 
                </a>
              @endif
              @if($ps->status == '2')
                <a href="{{ route('acInac') }}?id={{ $ps->id }}&val=1&tab=personas"> 
            ...
            <td>{{ $ps->name }}</td>
            <td>
              {{ route('viewTechResLst', array('lng' => $lngcode)) }}?catid={{ $ps->id }}
              {{-- url($lngcode.'/'.$ps->slug) --}}
            </td>
            <td>{{ date('d-m-Y', strtotime($ps->created_at)) }}</td>
            <td>{{ date('d-m-Y', strtotime($ps->updated_at)) }}</td>
          </tr>
          @php $sl++; @endphp
          @empty
          @endforelse
