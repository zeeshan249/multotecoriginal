...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('evt_crte_cat') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Create Category</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  ...
  <div class="box" style="margin-top: 10px;">
    <form name="frmx" action="{{ route('evtc.blkAct') }}" method="post">
    {{ csrf_field() }}
      ...
      <table class="table table-bordered table-hover table-striped ar-datatable display nowrap" style="width: 100%;">
        ...
        @if(isset($allCategories))
          ...
          <tr>
            ...
            <td>
              <a href="{{ route('evt_edt_cat', array('id' => $cat->id)) }}"><i class="fa fa-pencil-square-o fa-2x base-green"></i></a>

              <a href="{{ route('evt_del_cat', array('id' => $cat->id)) }}" onclick="return confirm('Sure To Delete This Category ?');"><i class="fa fa-trash-o fa-2x base-red"></i></a>
              <a href="{{ route('eventLists', array('lng' => $lngcode)) }}?catid={{ $cat->id }}" target="_blank">
                <i class="fa fa-eye fa-2x" aria-hidden="true"></i></a>
            </td>
            <td>
              @if($cat->status == '1')
                <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=2&tab=event_categories"> 
                  <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i> 
                </a>
              @endif
              @if($cat->status == '2')
                <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=1&tab=event_categories"> 
            ...
            <td>
              {{-- url($lngcode.'/'.$cat->slug) --}}
              {{ route('eventLists', array('lng' => $lngcode)) }}?catid={{ $cat->id }}
            </td> <!-- {{ url('/') }} -->
            <td>
              @if( isset($cat->TotalEvents) )
              {{ count( $cat->TotalEvents ) }}
              @endif
            </td>
