...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <a href="{{ route('edtProfile', array('id' => $parentLngCont->id)) }}" class="btn btn-primary"> Back</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  <div class="row" style="margin-top: 10px;">
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form name="jfrm" id="frmx" action="@if( isset($profile) ){{ route('adedLnProfilePst', array('pid' => $parentLngCont->id, 'cid' => $profile->id)) }}@else{{ route('adedLnProfilePst', array('pid' => $parentLngCont->id)) }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}

          <div class="row">
            @if(isset($profile) && isset($profile->Language)) 
            <div class="col-md-3">
              <div class="form-group">
                <div class="input-group">
                  <div class="input-group-addon">
                    <span>
                    <img src="{{ asset('public/uploads/flags/thumb/'. $profile->Language->flag) }}" style="height: 20px;" id="flag">
                    </span>
                  </div>
                  <select name="language_id" id="language_id" class="form-control">
                    <option value="{{ $profile->language_id }}">{{ $profile->Language->name }}</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            @if(isset($profile))
            <a href="{{ route('delLngCont', array('pid' => $parentLngCont->id, 'cid' => $profile->id)) }}" class="btn btn-danger pull-right" onclick="return confirm('Are You Sure Want To Delete This Content ?')">Delete This Content</a>
            @endif
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-10">
              ...
              <div class="form-group">
                <label>Short Description :</label>
                <textarea name="description" class="form-control" id="prof_desc">@if( isset($profile) ){{ html_entity_decode($profile->description, ENT_QUOTES) }}@endif</textarea>
              </div>
            </div>
          </div>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="follow" class="form-control">
                  <option value="0" @if(isset($profile) && $profile->follow == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($profile) && $profile->follow == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="index_tag" class="form-control">
                  <option value="0" @if(isset($profile) && $profile->index_tag == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($profile) && $profile->index_tag == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-10">
              <div class="form-group">
                ...
                @if( isset($profile) )
                {{--<span>
                  <a href="{{ route('previewTool') }}?url={{ route('preview', array('device' => 'desktop', 'slug' => $profile->slug)) }}&device=desktop" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Desktop Preview">
