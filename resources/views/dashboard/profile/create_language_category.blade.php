...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      ...
      <a href="{{ route('edtProfileCat', array('id' => $parentLngCont->id)) }}" class="btn btn-primary"> Back</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  <div class="row" style="margin-top: 10px;">
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form name="jfrm" id="frmx" action="@if( isset($profileCat) ){{ route('adedLnProfCatPst', array('pid' => $parentLngCont->id, 'cid' => $profileCat->id)) }}@else{{ route('adedLnProfCatPst', array('pid' => $parentLngCont->id)) }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}

          <div class="row">
            @if(isset($profileCat) && isset($profileCat->Language)) 
            <div class="col-md-3">
              <div class="form-group">
                <div class="input-group">
                  <div class="input-group-addon">
                    <span>
                    <img src="{{ asset('public/uploads/flags/thumb/'. $profileCat->Language->flag) }}" style="height: 20px;" id="flag">
                    </span>
                  </div>
                  <select name="language_id" id="language_id" class="form-control">
                    <option value="{{ $profileCat->language_id }}">{{ $profileCat->Language->name }}</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            @if(isset($profileCat))
            <a href="{{ route('delLngCatCont', array('pid' => $parentLngCont->id, 'cid' => $profileCat->id)) }}" class="btn btn-danger pull-right" onclick="return confirm('Are You Sure Want To Delete This Content ?')">Delete This Content</a>
            @endif
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-10">
              <div class="form-group">
                ...
                <input type="text" name="meta_title" class="form-control" placeholder="Meta Title" value="@if( isset($profileCat) ){{ $profileCat->meta_title }}@endif">
              </div>
              <div class="form-group">
                <label>Meta Keywords:</label>
                <input type="text" name="meta_keyword" class="form-control" placeholder="Meta Keywords" value="@if( isset($profileCat) ){{ $profileCat->meta_keyword }}@endif">
              </div>
              <div class="form-group">
                <label>Meta Description:</label>
                <textarea name="meta_desc" class="form-control" placeholder="Meta Description">@if( isset($profileCat) ){{ $profileCat->meta_desc }}@endif</textarea>
              </div>
              <div class="form-group">
                <label>Canonical Url:</label>
                <input type="text" name="canonical_url" class="form-control" placeholder="Any Canonical url" value="@if( isset($profileCat) ){{ $profileCat->canonical_url }}@endif">
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                <label>Language Tag:</label>
                <input type="text" name="lng_tag" class="form-control" placeholder="Language Tag" value="@if( isset($profileCat) ){{ $profileCat->lng_tag }}@endif">
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="follow" class="form-control">
                  <option value="0" @if(isset($profileCat) && $profileCat->follow == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($profileCat) && $profileCat->follow == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="index_tag" class="form-control">
                  <option value="0" @if(isset($profileCat) && $profileCat->index_tag == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($profileCat) && $profileCat->index_tag == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-10">
              <div class="form-group">
                ...
                @if( isset($profileCat) )
                {{--<span>
                  <a href="{{ route('previewTool') }}?url={{ route('preview', array('device' => 'desktop', 'slug' => $profileCat->slug)) }}&device=desktop" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Desktop Preview">
