  ...
  <div class="row" style="margin-top: 10px;">
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form name="frm" id="frmx" action="@if( isset($job) ){{ route('updJob', array('id' => $job->id)) }}@else{{ route('sveJob') }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}
          ...
          <div class="row">
            ...
            <div class="col-md-9" style="text-align: right;">
              @if( isset($job) && !empty($job) )
                @if( isset($job->ChildLanguages) && count($job->ChildLanguages) < count($languages) - 1 )
                <a href="{{ route('carr.adedlng', array('pid' => $job->id)) }}"><i class="fa fa-plus"></i> Add Language ?</a>
                @endif
                @if( isset($job->ChildLanguages) )
                  @foreach( $job->ChildLanguages as $chl )
                    @if( isset($chl->Language) )
                      @php $Lng = $chl->Language; @endphp
                      <a href="{{ route('carr.adedlng', array('pid' => $job->id, 'cid' => $chl->id)) }}" class="lngLink btn btn-xs">
