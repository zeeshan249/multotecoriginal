  ...
  <div class="row" style="margin-top: 10px;">
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form name="frm" id="frmx" action="@if(isset($user_data) && !empty($user_data)){{ route('upd_user', array('user_timestamp_id' => $user_data->timestamp_id)) }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                <label>User Role : <em>*</em></label>
                <select name="role_ids[]" id="role_ids" class="form-control" multiple="multiple">
                  <option value="">-Select Role-</option>
                  @if(isset($roles))
                    @foreach($roles as $rl)
                    <option value="{{ $rl->id }}" @if( isset($user_data) && $user_data->hasRole($rl->id) ) selected="selected" @endif>{{ ucwords($rl->name) }}</option>
                    @endforeach
                  @endif
                </select>
              </div>
