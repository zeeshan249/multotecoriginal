...
<section class="content-header">
      <h1>
        All Product Box Reusable Contents
...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('pbox_crte') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Create New Content</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  ...
  <div class="box" style="margin-top: 10px;">
    <form name="frmx" action="{{ route('pbox.blkAct') }}" method="post">
    {{ csrf_field() }}
      ...
      <table class="table table-bordered table-hover table-striped ar-datatable">
        ...
        @if(isset($contents))
          ...
          <tr>
            ...
            <td>
              <a href="{{ route('pbox_edit', array('id' => $cnt->id)) }}"><i class="fa fa-pencil-square-o base-green fa-2x"></i></a>
              <a href="{{ route('pbox_delete', array('id' => $cnt->id)) }}" onclick="return confirm('Sure To Delete This Content ?');"><i class="fa fa-trash-o base-red fa-2x"></i></a>
            </td>
            <td>
              @if($cnt->status == '1')
                <a href="{{ route('acInac') }}?id={{ $cnt->id }}&val=2&tab=productbox_reusable_content"> 
                  <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i> 
                </a>
              @endif
              @if($cnt->status == '2')
                <a href="{{ route('acInac') }}?id={{ $cnt->id }}&val=1&tab=productbox_reusable_content"> 
