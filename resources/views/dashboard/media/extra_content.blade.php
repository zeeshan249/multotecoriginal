    ...
    <div class="col-md-12">
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Add - Edit - Frontend Extra Content For Photos & Videos Page</h3>
        ...
        <div class="box-body">
          <form name="frm" id="frmx" action="{{ route('media.extra_cont_save') }}" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}
          <div class="row">
            <div class="col-md-10">
              <div class="form-group">
                <label>Photo Gallery Content : </label>
                <textarea name="photo_gallery_content" id="photo_gallery_content" class="form-control" data-error-container="#perror">@if(isset($extraCont)){{ html_entity_decode($extraCont->photo_gallery_content, ENT_QUOTES) }}@endif</textarea>
                <div id="perror"></div>
              </div>
              <div class="form-group">
                <label>Video Gallery Content : </label>
                <textarea name="video_gallery_content" id="video_gallery_content" class="form-control" data-error-container="#verror">@if(isset($extraCont)){{ html_entity_decode($extraCont->video_gallery_content, ENT_QUOTES) }}@endif</textarea>
                <div id="verror"></div>
              </div>
            </div>
          </div>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="follow" class="form-control">
                  <option value="0" @if(isset($extraCont) && $extraCont->follow == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($extraCont) && $extraCont->follow == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="index_tag" class="form-control">
                  <option value="0" @if(isset($extraCont) && $extraCont->index_tag == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($extraCont) && $extraCont->index_tag == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
          </div>
...
var editor_photo = CKEDITOR.replace( 'photo_gallery_content', {
  customConfig: "{{ asset('public/assets/ckeditor/mini_config.js') }}",
} );
var editor_video = CKEDITOR.replace( 'video_gallery_content', {
  customConfig: "{{ asset('public/assets/ckeditor/mini_config.js') }}",
} );
var fm = $('#frmx');
fm.on('submit', function() {
  CKEDITOR.instances.photo_gallery_content.updateElement();
  CKEDITOR.instances.video_gallery_content.updateElement();
});
fm.validate({
  errorElement: 'span',
  ...
  messages: {
    ...
    photo_gallery_content: {
      required: 'Please Enter Photos Content.'
    },
    video_gallery_content: {
      required: 'Please Enter Videos Content.'
    }
  },
