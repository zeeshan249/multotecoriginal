...
<section class="content-header">
      <h1>
        All Distributor Continents
        <!--small>it all starts here</small-->
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{ route('dashboard') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li><a href="{{ route('crteDistribCats') }}">Add New Category</a></li>
        <li class="active">All Continents</li>
      </ol>
    </section>
@endsection
...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('crteDistribCats') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add New Continent</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  ...
  <div class="box" style="margin-top: 10px;">
    <form name="frmx" action="{{ route('distrcat.blkAct') }}" method="post">
    {{ csrf_field() }}
    <div class="box-header with-border">
      <h3 class="box-title">All Continents</h3>
      ...
      <table class="table table-bordered table-hover table-striped display nowrap ar-datatable" style="width:100%">
        ...
        @if(isset($allCats))
          ...
          @forelse($allCats as $cat)
            @php
              $lngcode = getLngCode($cat->language_id);
            @endphp
          <tr>
            <td>
              {{ $sl }}
              <input type="checkbox" name="ids[]" class="ckbs" value="{{ $cat->id }}">
            </td>
            <td>
              <a href="{{ route('editDistribCats', array('id' => $cat->id)) }}"><i class="fa fa-pencil-square-o fa-2x base-green"></i></a>
              <a href="{{ route('delDistribCats', array('id' => $cat->id)) }}" onclick="return confirm('Sure To Delete This Category ?');"><i class="fa fa-2x fa-trash-o base-red"></i></a>
              <a href="{{ route('front.distrbCat', array('lng' => $lngcode, 'catslug' => $cat->slug)) }}" target="_blank">
              <i class="fa fa-eye fa-2x" aria-hidden="true"></i></a>
            </td>
            <td>
              @if($cat->status == '1')
                <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=2&tab=distributor_category"> 
                  <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i> 
                </a>
              @endif
              @if($cat->status == '2')
                <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=1&tab=distributor_category"> 
                  <i class="fa fa-ban base-red fa-2x" aria-hidden="true"></i>
                </a> 
              @endif
            </td>
            <td>{{ $cat->name }}</td>
            <td>@if( $cat->slug != '' ){{ route('front.distrbCat', array('lng' => $lngcode, 'catslug' => $cat->slug)) }}@endif</td>
