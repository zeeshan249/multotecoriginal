...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="{{ route('allDistribCats') }}">All Distributor Categories</a></li>
    @if(isset($category))
    <li class="active">Edit Distributor Category</li>
    @else
    <li class="active">Add Distributor Category</li>
    @endif
  </ol>
</section>
@endsection
...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('allDistribCats') }}" class="btn btn-primary"> All Distributor Categories</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
    ...
    <div class="col-md-12">
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">@if(isset($category)) Edit Distributor Category @else Add Distributor Category @endif</h3>
        ...
        <div class="box-body">
          <form name="jfrm" id="frmx" action="@if( isset($category) ){{ route('distrb.adedcatlngPst', array('pid' => $parentLngCont->id, 'cid' => $category->id)) }}@else{{ route('distrb.adedcatlngPst', array('pid' => $parentLngCont->id)) }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}
          ...
          <div class="row">
            ...
            @if(isset($category))
            <a href="{{ route('distrb.adedcatlngDel', array('pid' => $parentLngCont->id, 'cid' => $category->id)) }}" class="btn btn-danger pull-right" onclick="return confirm('Are You Sure Want To Delete This Content ?')">Delete This Content</a>
            @endif
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-10">
              <div class="form-group">
                ...
                <input type="text" name="meta_title" class="form-control" placeholder="Meta Title" value="@if( isset($category) ){{ $category->meta_title }}@endif">
              </div>
              <div class="form-group">
                <label>Meta Keywords:</label>
                <input type="text" name="meta_keyword" class="form-control" placeholder="Meta Keywords" value="@if( isset($category) ){{ $category->meta_keyword }}@endif">
              </div>
              <div class="form-group">
                <label>Meta Description:</label>
                <textarea name="meta_desc" class="form-control" placeholder="Meta Description">@if( isset($category) ){{ $category->meta_desc }}@endif</textarea>
              </div>
              <div class="form-group">
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="follow" class="form-control">
                  <option value="0" @if(isset($category) && $category->follow == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($category) && $category->follow == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="index_tag" class="form-control">
                  <option value="0" @if(isset($category) && $category->index_tag == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($category) && $category->index_tag == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
          </div>
...
$('#frmx').validate({
  ...
  messages: {
    ...
    name: {
      required: 'Please Enter Distributor Category Name.'
    },
    slug: {
