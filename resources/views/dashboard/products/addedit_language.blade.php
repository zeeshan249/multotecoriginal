  ...
  <div class="row" style="margin-top: 10px;">
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form name="jfrm" id="frmx" action="@if( isset($product) ){{ route('prod.adedlngPst', array('pid' => $parentLngCont->id, 'cid' => $product->id)) }}@else{{ route('prod.adedlngPst', array('pid' => $parentLngCont->id)) }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}

          <div class="row">
            @if(isset($product) && isset($product->Language)) 
            <div class="col-md-3">
              <div class="form-group">
                <div class="input-group">
                  <div class="input-group-addon">
                    <span>
                    <img src="{{ asset('public/uploads/flags/thumb/'. $product->Language->flag) }}" style="height: 20px;" id="flag">
                    </span>
                  </div>
                  <select name="language_id" id="language_id" class="form-control">
                    <option value="{{ $product->language_id }}">{{ $product->Language->name }}</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            @if(isset($product))
            <a href="{{ route('prod.adedlngDel', array('pid' => $parentLngCont->id, 'cid' => $product->id)) }}" class="btn btn-danger pull-right" onclick="return confirm('Are You Sure Want To Delete This Content ?')">Delete This Content</a>
            @endif
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-10">
              ...
              <div class="form-group">
                ...
                <input type="text" name="slug" id="pgSlug" class="form-control" placeholder="Enter Link or Page URL" value="@if( isset($product) ){{ $product->slug }}@endif" @if( isset($product) ) readonly="readonly" @endif>
              </div>
              <div class="form-group">
                <label>Short Description :</label>
                <textarea name="description" class="form-control">@if( isset($product) ){{ html_entity_decode($product->description, ENT_QUOTES) }}@endif</textarea>
              </div>
          ...
          <div class="row">
            <div class="col-md-10">
              <div class="form-group">
                ...
                @if( isset($product) )
                <span>
                  <a href="{{ route('previewTool') }}?url={{ route('preview', array('device' => 'desktop', 'slug' => $product->slug)) }}&device=desktop" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Desktop Preview">
                ...
                --}}
                <textarea name="page_content" class="form-control" id="pg_cont">@if( isset($product) ){{ html_entity_decode($product->page_content, ENT_QUOTES) }}@endif</textarea>
              </div>
            </div>
          </div>
