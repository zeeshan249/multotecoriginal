...
$( function() {
	var pgbFrmLoad = 0;
	$('body').on('click', '.pgb_eform', function() {

		$('#EFORM_this_id').val('0');
		$('select[name="device"]').val('3');
		$('#EFORM_main_title').val('');
		$('#EFORM_sub_title').val('');
		$("#EFORM_main_content").val('').trigger('change')
	...
	var pgb_eform_frm = $('#pgb_eform_frm');
	
	pgb_eform_frm.validate({
  		errorElement: 'span',
		...
		messages: {
		    ...
		    main_content: {
		      required: 'Please Select Any Form.'
		    },
		    insert_id: {
		        	...
		        	var html = '<div class="notice notice-info">';
		        			html += '<div class="row">';
		        				...
		        				html += '<div class="col-md-4 txtrit">';
		        					html += '<a href="javascript:void(0);" class="pgb_edt" id="' + EFORM_thisID + '" data="EFORM"><i class="fa fa-pencil base-green fa-2x" aria-hidden="true"></i></a>';
		        					html += '&nbsp;&nbsp;'
		        					html += '<a href="javascript:void(0);" class="pgb_del" id="' + EFORM_thisID + '" data="EFORM"><i class="fa fa-trash-o base-red fa-2x" aria-hidden="true"></i></a>';
		        				html += '</div>';
		        			html += '</div>';
		        		html += '</div>';

		        	if( action_status == 'insert' ) {
		        		$('#pgContentAppend').append('<div class="ar-order altTop '+ builder_type +'_holder_' + EFORM_thisID + '" id="' + EFORM_thisID +'">' + html + '</div>');
		        	}

		        	if( action_status == 'update' ) {
		        		$( '.'+ builder_type +'_holder_' + EFORM_thisID ).html( html );
		        	}
		        }
		        form.reset();
		      },
