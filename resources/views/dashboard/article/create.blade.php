...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="{{ route('allArts') }}">All Articles</a></li>
  </ol>
</section>
@endsection
...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('allArts') }}" class="btn btn-primary"> All Articles</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
    ...
    <div class="col-md-12">
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">@if(isset($article)) Edit Article @else Add Article @endif</h3>
        ...
        <div class="box-body">
          <form name="jfrm" id="frmx" action="@if( isset($article) ){{ route('updArt', array('id' => $article->id)) }}@else{{ route('sveArt') }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}
          ...
          <div class="row">
            ...
            <div class="col-md-9" style="text-align: right;">
              @if( isset($article) && !empty($article) )
                @if( isset($article->ChildLanguages) && count($article->ChildLanguages) < count($languages) - 1 )
                <a href="{{ route('arti.adedlng', array('pid' => $article->id)) }}"><i class="fa fa-plus"></i> Add Language ?</a>
                @endif
                @if( isset($article->ChildLanguages) )
                  @foreach( $article->ChildLanguages as $chl )
                    @if( isset($chl->Language) )
                      @php $Lng = $chl->Language; @endphp
                      <a href="{{ route('arti.adedlng', array('pid' => $article->id, 'cid' => $chl->id)) }}" class="lngLink btn btn-xs">
          ...
          <div class="row">
            <div class="col-md-10">
              <div class="form-group">
                <label>Article Name (H1) : <em>*</em></label>
                <input type="text" name="name" id="artName" class="form-control" placeholder="Enter Article Name" value="@if( isset($article) ){{ $article->name }}@endif">
              </div>
              <div class="form-group">
                <label>Link or Url : <em>*</em></label>
                <input type="text" name="slug" id="pgSlug" class="form-control" placeholder="Enter Link or Page URL" value="@if( isset($article) ){{ $article->slug }}@endif" @if( isset($article) ) readonly="readonly" @endif>
              </div>
              <div class="form-group">
                <label>Short Description :</label>
                <textarea name="description" class="form-control" id="prod_desc">@if( isset($article) ){{ html_entity_decode($article->description, ENT_QUOTES) }}@endif</textarea>
              </div>

              <div class="form-group">
                @php
                  if( isset($article) && isset($article->categoryIds) && count($article->categoryIds) > 0 ) {
                    $seleArr = array();
                    foreach($article->categoryIds as $v) {
                      array_push($seleArr, $v->article_category_id);
                    }
                  }
                  @endphp
                  <label>Select Category :</label>
                  <select name="article_category_ids[]" class="form-control select2">
                    <option value="0">-Select Article or News Category-</option>
                    @if( isset($allArtCats) )
                      @foreach( $allArtCats as $cat )
                      <option value="{{ $cat->id }}" @if( isset($article) && isset($seleArr) && in_array($cat->id, $seleArr) ) selected="selected" @endif>{{ $cat->name }}</option>
                      @endforeach
                    @endif
                  </select>
              </div>
          ...
          <div class="row">
            <div class="col-md-4">
              <div class="form-group">
                ...
                <input type="text" name="publish_date" class="form-control datepicker" placeholder="Publish Date" value="@if( isset($article) ){{ date('d-m-Y', strtotime($article->publish_date)) }}@endif">
              </div>
            </div>
            <div class="col-md-4">
              <div class="form-group">
                <label>Display Order :</label>
                <input type="text" name="display_order" class="form-control onlyNumber" style="width: 100px;" @if( isset($article) ) value="{{ $article->display_order }}" @else value="0" @endif>
              </div>
            </div>
          </div>
            ...
            <div class="row">
              ...
              <div class="col-md-10">
                <div class="form-group">
                  ...
                  <input type="text" name="meta_title" class="form-control" placeholder="Meta Title" value="@if( isset($article) ){{ $article->meta_title }}@endif">
                </div>
                <div class="form-group">
                  <label>Meta Keywords:</label>
                  <input type="text" name="meta_keyword" class="form-control" placeholder="Meta Keywords" value="@if( isset($article) ){{ $article->meta_keyword }}@endif">
                </div>
                <div class="form-group">
                  <label>Meta Description:</label>
                  <textarea name="meta_desc" class="form-control" placeholder="Meta Description">@if( isset($article) ){{ $article->meta_desc }}@endif</textarea>
                </div>
                <div class="form-group">
                  <label>Canonical Url:</label>
                  <input type="text" name="canonical_url" class="form-control" placeholder="Any Canonical url" value="@if( isset($article) ){{ $article->canonical_url }}@endif">
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
                  <label>Language Tag:</label>
                  <input type="text" name="lng_tag" class="form-control" placeholder="Language Tag" value="@if( isset($article) ){{ $article->lng_tag }}@endif">
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <select name="follow" class="form-control">
                    <option value="1" @if(isset($article) && $article->follow == '1') selected="selected" @endif>FOLLOW</option>
                    <option value="0" @if(isset($article) && $article->follow == '0') selected="selected" @endif>NO FOLLOW</option>
                  </select>
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
                  ...
                  <select name="index_tag" class="form-control">
                    <option value="1" @if(isset($article) && $article->index_tag == '1') selected="selected" @endif>INDEX</option>
                    <option value="0" @if(isset($article) && $article->index_tag == '0') selected="selected" @endif>NO INDEX</option>
                  </select>
                </div>
              </div>
              <div class="col-md-10">
                <div class="form-group">
                  ...
                  <textarea name="json_markup" class="form-control" rows="6">@if( isset($article) ){!! html_entity_decode($article->json_markup, ENT_QUOTES) !!}@endif</textarea>
                </div>
              </div>
            </div>
            <!-- END META INFO -->
          ...
          <div class="row">
            <div class="col-md-10">
              <div class="form-group">
                ...
                <textarea name="page_content" class="form-control" id="pg_cont">@if( isset($article) ){{ html_entity_decode($article->page_content, ENT_QUOTES) }}@endif</textarea>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <!-- CALL SCRIPT -->
                <button type="button" class="btn btn-default addMedImgBtn" title="Article Image" data="articleImage"><i class="fa fa-picture-o" aria-hidden="true"></i> Add Article Image</button>
                
                <input type="hidden" id="articleImage-idholder" name="article_image_ids">

                <input type="hidden" id="articleImage-infoholder" name="article_image_infos">
                
                <div id="articleImage-dispDiv"></div>
                <!-- END -->
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <!-- CALL SCRIPT -->
                <button type="button" class="btn btn-default addMedImgBtn" title="Article Thumb Image" data="thumbImage"><i class="fa fa-picture-o" aria-hidden="true"></i> Add Article Thumb Image</button>
          ...
          <div class="row">
              <div class="col-md-6">
              @if( isset($article) && isset($article->allImgIds) && count($article->allImgIds) > 0 )
                @php $i = 0; @endphp
                @foreach( $article->allImgIds as $imgs )
                @if( $imgs->image_type == 'MAIN_IMAGE' )
                  ...
                  <div class="thumbnail">
                    ...
                    <a href="javascript:void(0);" class="idel ifdel" data="article_images_map" id="{{ $imgs->id }}">
          ...
          <div class="row" style="margin-top: 20px;">
            <div class="col-md-6">
              ...
              <input type="hidden" id="table_id" value="{{ $article->id }}">
              <input type="hidden" name="insert_id" id="insert_id" value="{{ $article->insert_id }}">
              @else
              <!--input type="submit" class="btn btn-primary" value="Save All"-->
              <!--a href="{{ route('addArt') }}" class="btn btn-danger">Cancel</a-->
...
$( function() {
  ...
  $('#article_category_ids').multiselect({
    buttonWidth : '100%',
