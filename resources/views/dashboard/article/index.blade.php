...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('addArt') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add New Article</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  ...
  <div class="box" style="margin-top: 10px;">
    <form name="frmx" action="{{ route('art.blkAct') }}" method="post">
    {{ csrf_field() }}
      ...
      <table class="table table-bordered table-hover table-striped ar-datatable display nowrap" style="width: 100%;">
        ...
        @if(isset($allArticles))
          ...
          @forelse($allArticles as $art)
            @php
              $lngcode = getLngCode($art->language_id);
            @endphp
          <tr>
            <td>
              {{ $sl }}
              <input type="checkbox" name="ids[]" class="ckbs" value="{{ $art->id }}">
            </td>
            <td>
              <a href="{{ route('edtArt', array('id' => $art->id)) }}"><i class="fa fa-pencil-square-o base-green fa-2x"></i></a>
              <a href="{{ route('delArt', array('id' => $art->id)) }}" onclick="return confirm('Sure To Delete This Article ?');"><i class="fa fa-trash-o fa-2x base-red"></i></a>
              <a href="{{ route('front.artCont', array('lng' => 'en', 'slug' => $art->slug)) }}" target="_blank">
              <i class="fa fa-eye fa-2x" aria-hidden="true"></i></a>
            </td>
            <td>
              @if($art->status == '1')
                <a href="{{ route('acInac') }}?id={{ $art->id }}&val=2&tab=articles"> 
                  <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i> 
                </a>
              @endif
              @if($art->status == '2')
                <a href="{{ route('acInac') }}?id={{ $art->id }}&val=1&tab=articles"> 
                  <i class="fa fa-ban base-red fa-2x" aria-hidden="true"></i>
                </a> 
              @endif
            </td>
            <td>{{ ucfirst( $art->name ) }}</td>
            <td>
              @if( isset($art->categoryIds) && count($art->categoryIds) > 0 )
                @foreach( $art->categoryIds as $cts )
            ...
            <td>{{ route('front.artCont', array('lng' => 'en', 'slug' => $art->slug)) }}</td>
