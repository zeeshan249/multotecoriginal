...
<section class="content-header">
      <h1>
        All Industry Flowsheet Categories
        <!--small>it all starts here</small-->
      </h1>
      <ol class="breadcrumb">
        <li><a href="{{ route('dashboard') }}"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">All Flowsheet Categories</li>
      </ol>
    </section>
@endsection
...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('addFSc') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add New Category</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
  ...
  <div class="box" style="margin-top: 10px;">
    <form name="frmx" action="{{ route('ifsc.blkAct') }}" method="post">
    {{ csrf_field() }}
    <div class="box-header with-border">
      <h3 class="box-title">All Flowsheet Categories</h3>
      ...
      <table class="table table-bordered table-hover table-striped display nowrap ar-datatable" style="width:100%">
        ...
        @if(isset($allCats))
          ...
          <tr>
            ...
            <td>
              <a href="{{ route('edtFSc', array('id' => $cat->id)) }}"><i class="fa fa-pencil-square-o fa-2x base-green"></i></a>
              <a href="{{ route('delFSc', array('id' => $cat->id)) }}" onclick="return confirm('Sure To Delete This Category ?');"><i class="fa fa-2x fa-trash-o base-red"></i></a>
              <a href="{{ url($lngcode.'/'.$cat->slug) }}" target="_blank">
              <i class="fa fa-eye fa-2x" aria-hidden="true"></i></a>
            </td>
            <td>
              @if($cat->status == '1')
                <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=2&tab=flowsheet_category"> 
                  <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i> 
                </a>
              @endif
              @if($cat->status == '2')
                <a href="{{ route('acInac') }}?id={{ $cat->id }}&val=1&tab=flowsheet_category"> 
                  <i class="fa fa-ban base-red fa-2x" aria-hidden="true"></i>
                </a> 
              @endif
            </td>
            <td>{{ ucfirst( $cat->name ) }}</td>
            <td>@if( $cat->slug != '' ){{ url($lngcode.'/'.$cat->slug) }}@endif</td>
