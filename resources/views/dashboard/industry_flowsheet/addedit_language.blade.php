  ...
  <div class="row" style="margin-top: 10px;">
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form name="jfrm" id="frmx" action="@if( isset($flowsheet) ){{ route('indfs.adedlngPst', array('pid' => $parentLngCont->id, 'cid' => $flowsheet->id)) }}@else{{ route('indfs.adedlngPst', array('pid' => $parentLngCont->id)) }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}

          <div class="row">
            @if(isset($flowsheet) && isset($flowsheet->Language)) 
            <div class="col-md-3">
              <div class="form-group">
                <div class="input-group">
                  <div class="input-group-addon">
                    <span>
                    <img src="{{ asset('public/uploads/flags/thumb/'. $flowsheet->Language->flag) }}" style="height: 20px;" id="flag">
                    </span>
                  </div>
                  <select name="language_id" id="language_id" class="form-control">
                    <option value="{{ $flowsheet->language_id }}">{{ $flowsheet->Language->name }}</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            @if(isset($flowsheet))
            <a href="{{ route('indfs.adedlngDel', array('pid' => $parentLngCont->id, 'cid' => $flowsheet->id)) }}" class="btn btn-danger pull-right" onclick="return confirm('Are You Sure Want To Delete This Content ?')">Delete This Content</a>
            @endif
            </div>
          </div>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="follow" class="form-control">
                  <option value="0" @if(isset($flowsheet) && $flowsheet->follow == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($flowsheet) && $flowsheet->follow == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="index_tag" class="form-control">
                  <option value="0" @if(isset($flowsheet) && $flowsheet->index_tag == '0') selected="selected" @endif>NO</option>
                  <option value="1" @if(isset($flowsheet) && $flowsheet->index_tag == '1') selected="selected" @endif>YES</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-10">
              ...
              <div class="form-group">
                <label>Select Flowsheet Categories :</label>
                @php
                ...
                @endphp
                <select name="flowsheet_category_id[]" id="flowsheet_category_id" class="form-control">
                  @if( isset($allCats) )
              ...
              <div class="form-group">
                ...
                @if( isset($flowsheet) )
                {{--<span>
                  <a href="{{ route('previewTool') }}?url={{ route('preview', array('device' => 'desktop', 'slug' => $flowsheet->slug)) }}&device=desktop" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Desktop Preview">
