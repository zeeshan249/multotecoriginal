  ...
  <div class="row" style="margin-top: 10px;">
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form name="jfrm" id="frmx" action="@if( isset($fscategory) ){{ route('updFSc', array('id' => $fscategory->id)) }}@else{{ route('sveFSc') }}@endif" method="post" enctype="multipart/form-data">
          {{ csrf_field() }}
          ...
          <div class="row">
            ...
            <div class="col-md-9" style="text-align: right;">
              @if( isset($fscategory) && !empty($fscategory) )
                @if( isset($fscategory->ChildLanguages) && count($fscategory->ChildLanguages) < count($languages) - 1 )
                <a href="{{ route('indfs.adedcatlng', array('pid' => $fscategory->id)) }}"><i class="fa fa-plus"></i> Add Language ?</a>
                @endif
                @if( isset($fscategory->ChildLanguages) )
                  @foreach( $fscategory->ChildLanguages as $chl )
                    @if( isset($chl->Language) )
                      @php $Lng = $chl->Language; @endphp
                      <a href="{{ route('indfs.adedcatlng', array('pid' => $fscategory->id, 'cid' => $chl->id)) }}" class="lngLink btn btn-xs">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              @if( isset($fscategory) && $fscategory->image_id != '' && isset($fscategory->imageInfo) )
                <div class="form-group">
                  <img src="{{ asset('public/uploads/files/media_images/'.$fscategory->imageInfo->image) }}" style="width: 260px; height: 100px;">
                  <a href="{{ route('glbImgDel') }}?tab=flowsheet_category&id={{ $fscategory->id }}" class="btn btn-xs btn-danger" onclick="return confirm('Are you sure to delete this image ?');">Delete Image</a>
                </div>
              @endif
              <div class="form-group">
                <label>Banner Title :</label>
                <input type="text" name="image_title" class="form-control" placeholder="Banner image title" value="@if( isset($fscategory) ){{ $fscategory->image_title }}@endif">
              </div>
              <div class="form-group">
                <label>Banner Alt Tag :</label>
                <input type="text" name="image_alt" class="form-control" placeholder="Banner image alt title" value="@if( isset($fscategory) ){{ $fscategory->image_alt }}@endif">
              </div>
              <div class="form-group">
                <label>Banner Caption :</label>
                <textarea name="image_caption" class="form-control" placeholder="Banner image caption">@if( isset($fscategory) ){{ $fscategory->image_caption }}@endif</textarea>
              </div>
            </div>
          </div>
            ...
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="follow" class="form-control">
                  <option value="1" @if(isset($fscategory) && $fscategory->follow == '1') selected="selected" @endif>FOLLOW</option>
                  <option value="0" @if(isset($fscategory) && $fscategory->follow == '0') selected="selected" @endif>NO FOLLOW</option>
                </select>
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                ...
                <select name="index_tag" class="form-control">
                  <option value="1" @if(isset($fscategory) && $fscategory->index_tag == '1') selected="selected" @endif>INDEX</option>
                  <option value="0" @if(isset($fscategory) && $fscategory->index_tag == '0') selected="selected" @endif>NO INDEX</option>
                </select>
              </div>
            </div>
            <div class="col-md-10">
              <div class="form-group">
                ...
                <textarea name="json_markup" class="form-control" rows="6">@if( isset($fscategory) ){!! html_entity_decode($fscategory->json_markup, ENT_QUOTES) !!}@endif</textarea>
              </div>
            </div>
          </div>
          <!-- END META INFO -->
