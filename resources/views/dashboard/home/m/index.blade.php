...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-6">
      <a href="{{ route('mina_add') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> Add New</a>
    </div>
    <div class="col-md-6">
    </div>
  </div>
      ...
      <table class="table table-bordered table-hover table-striped ar-datatable display nowrap" style="width: 100%;">
        ...
        @if(isset($allList))
          ...
          <tr>
            ...
            <td>
              <a href="{{ route('mina_edt', array('id' => $v->id)) }}"><i class="fa fa-pencil-square-o base-green fa-2x"></i></a>
              <a href="{{ route('mina_del', array('id' => $v->id)) }}" onclick="return confirm('Sure To Delete This ?');"><i class="fa fa-trash-o fa-2x base-red"></i></a>
            </td>
            <td>
              @if($v->status == '1')
                <a href="{{ route('acInac') }}?id={{ $v->id }}&val=2&tab=mineral"> 
                  <i class="fa fa-check-circle-o base-green fa-2x" aria-hidden="true"></i> 
                </a>
              @endif
              @if($v->status == '2')
                <a href="{{ route('acInac') }}?id={{ $v->id }}&val=1&tab=mineral"> 
